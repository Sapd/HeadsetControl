...
int main(int argc, char* argv[])
{
    ...
    int daemonize                        = 0;
    //int sidetone_loudness                = -1; set globally for now bc. of corsair void daemon test
...
#define BUFFERLENGTH 1024
    ...
    while ((c = getopt_long(argc, argv, "bcdhi:l:f::mn:r:s:uv:p:e:?", opts, &option_index)) != -1) {
        ...
        case 0:
            if (strcmp(opts[option_index].name, "dev") == 0) {
                ...
                break;
            } else if (strcmp(opts[option_index].name, "debug") == 0) {
                debug_info = 1;
                break;
            }
            // fall through
...
loop_start:
    ...
    if (daemonize != -1) {
        if ((error = handle_feature(&device_found, &device_handle, &hid_path, CAP_DAEMONIZE, 0)) != 0)
            goto error;
    }
