...
enum voidpro_wireless_battery_flags {
    VOIDPRO_WIRELESS_BATTERY_MICUP = 128
};
...
static int voidpro_wireless_request_battery(hid_device *device_handle)
{
    ...
    else if (data_read[4] == 1)
    {
        ...
        if (data_read[2] & VOIDPRO_WIRELESS_BATTERY_MICUP)
            return data_read[2] &~ VOIDPRO_WIRELESS_BATTERY_MICUP;
        else
...
static int voidpro_wireless_notification_sound(hid_device* device_handle, uint8_t soundid)
{
